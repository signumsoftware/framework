...
class Upgrade_20211006_CleanProgram : CodeUpgradeBase
{
    ...
    public override void Execute(UpgradeContext uctx)
    {
        uctx.ChangeCodeFile(@"Southwind.Terminal\Program.cs", file =>
        {
            ...
            file.ReplaceBetween(new(a => a.Contains("public static void NewDatabase()"), +0), new(a => a.Contains(@"Console.WriteLine(""Done."");"), +1), "");
            file.ReplaceBetween(new(a => a.Contains("static void Synchronize()"), +0), new(a => a.Contains(@"command.OpenSqlFileRetry();"), +1), "");
        });
    }
}
