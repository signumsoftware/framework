...
export namespace EntityOperations {
  ...
  export function withIcon(text: string, icon?: IconProp, iconColor?: string, iconAlign?: "start" | "end"): string | React.ReactElement {
    if (icon) {
      switch (iconAlign) {
        case "end": return (<span>{text}<FontAwesomeIcon aria-hidden={true} icon={icon} color={iconColor} fixedWidth className="ms-2" /></span>);
        default: return (<span><FontAwesomeIcon aria-hidden={true} icon={icon} color={iconColor} fixedWidth className="me-2" />{text}</span>);
      }
    }
    else {
      return text;
        ...
        <div className="btn-group"
          ...
          {buttonAlternatives.map((aos, i) =>
            <Button key={i}
              ...
              <small><FontAwesomeIcon aria-hidden={true} icon={aos.icon!} color={aos.iconColor} fixedWidth /></small>
            </Button>
          )}
        </div>
      );
