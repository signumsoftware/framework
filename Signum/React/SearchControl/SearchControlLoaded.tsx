...
function SearchControlEllipsisMenu(p: { sc: SearchControlLoaded, isHidden: boolean }) {
  ...
  return (
    <Dropdown as={ButtonGroup} title={SearchMessage.Filters.niceToString()}>
      <Button variant="light" id="" className="sf-filter-button" aria-label={SearchMessage.Filters.niceToString()} active={active} onClick={e => p.sc.handleChangeFiltermode(active ? 'Simple' : 'Advanced')}>
        <FontAwesomeIcon icon="filter" /> {activeFilters == 0 ? null : activeFilters}
      </Button>
      <Dropdown.Toggle split className="px-2" variant={"light"} aria-label={SearchMessage.FilterTypeSelection.niceToString()}></Dropdown.Toggle>
