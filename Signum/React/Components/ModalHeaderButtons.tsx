  ...
  function renderButton(text: string, mip?: ModalIconProps) {
    if (mip?.icon) {
      switch (mip.iconAlign) {
        case "right": return (<span>{text} <FontAwesomeIcon aria-hidden={true} icon={mip.icon} fixedWidth /></span>);
        default: return (<span><FontAwesomeIcon aria-hidden={true} icon={mip.icon} fixedWidth /> {text}</span>);
      }
    }
    else
      return text;
