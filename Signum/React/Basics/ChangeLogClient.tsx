...
export async function getChangeLogs() : Promise<ChangeItem[]>{
  ...
  var modLogs = modules.flatMap((m, i) => {
    return Object.entries(m.dic).map<ChangeItem>(([date, changeLog]) => {
      return { module: m.modName, implDate: date, deployDate: null! as string, changeLog: changeLog };
    });
  });

  var mainLogs = Object.entries(mainLog).map<ChangeItem>(([date, changeLog]) => {
    return { module: Options.applicationName, deployDate: date, implDate: date, changeLog: changeLog };
  });
 ...
 mainLogs.orderBy(log => log.deployDate).forEach((log, i) => {
    ...
    var array = typeof log.changeLog == "string" ? [log.changeLog] : log.changeLog;

    array.forEach(cl => {
       
        var a  = /Update (?<mod>\w+)( to (?<date>\d{4}.\d{2}.\d{2}))?/.exec(cl);
