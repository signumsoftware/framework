...
namespace Signum.Engine
{
    public class SqlBuilder
    {
        ...
        public SqlPreCommand CreateTableSql(ITable t)
        {
            ...
            var columns = t.Columns.Values.Select(c => this.CreateColumn(c, GetDefaultConstaint(t, c), isChange: false))
...
FOR EACH ROW EXECUTE PROCEDURE versioning(
  'sys_period', '{t.SystemVersioned.TableName}', true
        ...
        public string CreateColumn(IColumn c, DefaultConstraint? constraint, bool isChange)
        {
            string fullType = GetColumnType(c);

            var generatedAlways = c is SystemVersionedInfo.SqlServerPeriodColumn svc ?
