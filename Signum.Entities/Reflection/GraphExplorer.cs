...
namespace Signum.Entities.Reflection
{
    public static class GraphExplorer
    {
        ...
        public static Dictionary<Modifiable, string> GraphIntegrityCheckDictionary(this Modifiable modificable, Func<Modifiable, IEnumerable<Modifiable>> explorer)
        {
        ...
        public static string Integrity(DirectedGraph<Modifiable> modifiable)
        {
            var problems = (from m in modifiable.OfType<IdentifiableEntity>()
                            group m by new { Type = m.GetType(), Id = (m as IdentifiableEntity).TryCS(ident => ident.IdOrNull) ?? m.temporalId } into g
