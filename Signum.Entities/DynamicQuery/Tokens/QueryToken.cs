    ...
    public abstract class QueryToken : IEquatable<QueryToken>
    {
        ...
        protected List<QueryToken> SubTokensBase(Type type, Implementations? implementations)
        {
            ...
            if (cleanType.IsIIdentifiable())
            {
                ...
                if(implementations.Value.Types.Length != 1)
                ...
                var singleType = implementations.Value.Types.Single();

                return new[] { EntityPropertyToken.IdProperty(this), EntityPropertyToken.ToStrProperty(this) }
                    .Concat(EntityProperties(singleType).Concat(OnEntityExtension(singleType, this)).OrderBy(a => a.ToString())).ToList();
            }
