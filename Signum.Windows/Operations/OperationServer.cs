...
namespace Signum.Windows
{
    public static class OperationServer
    {
        ...
        {
            return (T)(IEntity)Server.Return((IOperationServer s) => s.ExecuteOperation(entity, symbol.Symbol, args)); 
        }
        ...
        {
            return (T)(IEntity)Server.Return((IOperationServer s) => s.ConstructFrom(entity, symbol.Symbol, args)); 
        }
        ...
        public static T ConstructFromLite<F, FB, T>(this Lite<F> lite, ConstructSymbol<T>.From<FB> symbol, params object[] args)
            ...
            where F : class, IEntity, FB
        {
            return (T)(IEntity)Server.Return((IOperationServer s) => s.ConstructFromLite(lite, symbol.Symbol, args)); 
        }
        ...
        public static T ConstructFromMany<F, FB, T>(List<Lite<F>> lites, ConstructSymbol<T>.FromMany<FB> symbol, params object[] args)
            ...
            where F : class, IEntity, FB
        {
            return (T)(IEntity)Server.Return((IOperationServer s) => s.ConstructFromMany(lites, typeof(F), symbol.Symbol, args)); 
        }
    }
}
