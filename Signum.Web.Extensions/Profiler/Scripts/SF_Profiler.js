...
SF.Profiler = (function () {
    ...
    var heavyDetailsChart = function (data, currentDepth) {
        var fontSize = 12;
        var fontPadding = 3;
        var minDepth = d3.min($.map(data, function (e) { return e.Depth; }));
        var maxDepth = d3.max($.map(data, function (e) { return e.Depth; }));
        ...
        var chart = d3.select('.sf-profiler-chart')
            .append('svg:svg').attr('width', width).attr('height', height);

        var groups = chart.selectAll("g.entry").data(data).enter()
            .append('svg:g').attr('class', 'entry')
            .attr('data-full-index', function (v) { return v.FullIndex; });

        var rectangles = groups.append('svg:rect').attr('class', 'shape')
            .attr('x', function (v) { return v.Depth < currentDepth ? x(currentEntry.BeforeStart) : x(v.BeforeStart); })
            .attr('y', function (v) { return y(v.Depth); })
        ...
        groups.append('svg:rect').attr('class', 'shape-before')
            ...
            .attr('y', function (v) { return v.Depth < currentDepth ? 0 : y(v.Depth) + 1; })
            .attr('width', function (v) { return v.Depth < currentDepth ? 0 : x(v.Start) - x(v.BeforeStart); })
            .attr('height', function (v) { return v.Depth < currentDepth ? 0 : entryHeight - 2; })
            .attr('fill', '#fff');

        var labelsTop = groups.append('svg:text').attr('class', 'label label-top')
            .attr('dx', function (v) { return v.Depth < currentDepth ? x(currentEntry.Start) + 3 : x(v.Start) + 3; })
        ...
        var labelsBottom = groups.append('svg:text').attr('class', 'label label-bottom')
            .attr('dx', function (v) { return v.Depth < currentDepth ? x(currentEntry.Start) + 3 : x(v.Start) + 3; })
            .attr('dy', function (v) { return y(v.Depth); })
    ...
    var heavyListChart = function (data) {
        var fontSize = 12;
        var fontPadding = 4;
        var characterWidth = 7;
        var labelWidth = 60 * characterWidth; //Max characters: 100
        var rightMargin = 10 * characterWidth; //Aproximate elapsed time length: 10
        ...
        var labelsLeft = groups.append('svg:text').attr('class', 'label label-left')
        ...
        var labelsRight = groups.append('svg:text').attr('class', 'label label-right')
    ...
    return {
        initStats: initStats,
        init: init,
        heavyDetailsChart: heavyDetailsChart,
        heavyListChart: heavyListChart
    };
})(); 