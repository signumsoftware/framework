...
(function (SF) {
    (function (Chart) {
        (function (Utils) {
            ...
            Utils.fillAllTokenValueFuntions = fillAllTokenValueFuntions;
            ...
            Utils.makeItTokenValue = makeItTokenValue;
            ...
            Utils.getClickKeys = getClickKeys;
            ...
            function scaleFor(column, values, minRange, maxRange, scaleName) {
                ...
                if (scaleName == "MinMax")
                    return ((column.type == "Date" || column.type == "DateTime") ? d3.time.scale() : d3.scale.linear()).domain([
                        d3.min(values),
                        d3.max(values)]).range([minRange, maxRange]);

                if (scaleName == "Log")
                    return d3.scale.log().domain([
                        d3.min(values),
                        d3.max(values)]).range([minRange, maxRange]);

                if (scaleName == "Sqrt")
                    return d3.scale.pow().exponent(.5).domain([
                        d3.min(values),
                        d3.max(values)]).range([minRange, maxRange]);

                throw Error("Unexpected scale: " + scaleName);
            }
            Utils.scaleFor = scaleFor;
                ...
                return {
                    ...
                    debugX: function (chart) {
                        ...
                        chart.append('svg:g').attr('class', 'x-axis-rule-label').enterData(keys, 'text', 'x-axis-rule-label').attr('transform', function (d, i) {
                            return SF.Chart.Utils.translate(starts[d] + sizes[d] / 2 - 5, 10 + 100 * (i % 3)) + SF.Chart.Utils.rotate(90);
                    ...
                    debugY: function (chart) {
                        ...
                        chart.append('svg:g').attr('class', 'y-axis-rule-label').enterData(keys, 'text', 'y-axis-rule-label').attr('transform', function (d, i) {
                            return SF.Chart.Utils.translate(100 * (i % 3), starts[d] + sizes[d] / 2 + 4);
...
//# sourceMappingURL=SF_Chart_Utils.js.map
