    ...
    function scaleFor(column, values, minRange, maxRange, scaleName) {
        ...
        if (scaleName == "MinMax") {
            if (column.type == "Date" || column.type == "DateTime") {
                var scale = d3.time.scale().domain([new Date(d3.min(values)), new Date(d3.max(values))]).range([minRange, maxRange]);
                ...
                return d3.scale.linear().domain([d3.min(values), d3.max(values)]).range([minRange, maxRange]);
            }
        }
