...
import { useController } from '../Lines/LineBase'

export interface ValueSearchControlProps {
  ...
  modalSize?: BsSize;
  onRender?: (value: any | undefined, vsc: ValueSearchControlController) => React.ReactElement;
...
export interface ValueSearchControlController {
  props: ValueSearchControlProps;
...
const ValueSearchControl = React.forwardRef(function ValueSearchControl(p: ValueSearchControlProps, ref: React.Ref<ValueSearchControlController>): React.ReactElement | null {
  ...
  }, [p.initialValue, Finder.findOptionsPath(p.findOptions), ...(deps || [])], { avoidReset: true });

  function refreshValue() {
    setReloadTicks(a => a + 1);
  }

  var controller = React.useMemo(() => ({} as any as ValueSearchControlController), []);
  controller.props = p;
...
(ValueSearchControl as any).defaultProps = {
  isLink: false,
  isBadge: "MoreThanZero"
} as Partial<ValueSearchControlProps>;

export default ValueSearchControl;
