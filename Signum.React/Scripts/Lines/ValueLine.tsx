...
function numericSpinnerTextBox(vl: ValueLineController, validateKey: (e: React.KeyboardEvent<any>) => boolean) {
    ...
    const minusOnClick = (e: React.MouseEvent<any>) => {
        e.preventDefault();

        vl.setValue((!s.ctx.value ? 0 : s.ctx.value as number) - 1);
    }

    const plusOnClick = (e: React.MouseEvent<any>) => {
        e.preventDefault();
        ...
        <FormGroup ctx={s.ctx} labelText={s.labelText} helpText={s.helpText} htmlAttributes={{ ...vl.baseHtmlAttributes(), ...s.formGroupHtmlAttributes }} labelHtmlAttributes={s.labelHtmlAttributes}>
            <div className="input-group">
                <span className="input-group-btn">
                    <button type="button" className="quantity-left-minus btn btn-number" onClick={minusOnClick}>
                ...
                {vl.withItemGroup(
                    <NumericTextBox
                        ...
                        format={numbroFormat} />)}
                <span className="input-group-btn">
                    <button type="button" className="quantity-right-plus btn btn-number" onClick={plusOnClick}>
