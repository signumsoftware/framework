...
MOVE '{4}' TO '{5}'".Formato(databaseName, backupFile,
        ...
        {
            Executor.ExecuteNonQuery("ALTER TABLE {0} NOCHECK CONSTRAINT all".Formato(table.Name.SqlScape()));
        }
        ...
        public static int? MaxIdInRange(ITable table, int seedMin, int seedMax)
        {
            ...
            int? max = (int?)Executor.ExecuteScalar("SELECT MAX(Id) FROM {0} WHERE @min <= Id AND Id < @max".Formato(table.Name.SqlScape()), new List<DbParameter>
            {
        ...
        public static SeedInfo GetSeedInfo(ITable table)
        {
            using (Transaction tr = new Transaction())
            {
                ...
                Executor.ExecuteNonQuery("DBCC CHECKIDENT ({0}, NORESEED)".Formato(table.Name.SqlScape()));
        ...
        public static int GetNextId(ITable table)
        {
            ...
            return Executor.ExecuteNonQuery("SELECT IDENT_CURRENT ('{0}') ".Formato(table.Name.SqlScape()));
        }
        ...
        public static void SetNextId(ITable table, int nextId)
        {
            ...
            Executor.ExecuteNonQuery("DBCC CHECKIDENT ({0}, RESEED, @seed)".Formato(table.Name.SqlScape()), new List<DbParameter>
            {
