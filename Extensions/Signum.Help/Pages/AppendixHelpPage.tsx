...
import { EditableText, EditableHtml } from '../Editor/Editable';
...
function DeleteButton({ ctx }: { ctx: TypeContext<AppendixHelpEntity> }) {
  ...
  return <button type="button" className="btn btn-danger ms-4" onClick={onClick}><FontAwesomeIcon aria-hidden={true} icon="trash" /> {getOperationInfo(AppendixHelpOperation.Delete, AppendixHelpEntity).niceName}</button>;
}
