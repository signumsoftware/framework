...
import { AutoLine, EntityRepeater, EntityTable, TextBoxLine } from '@framework/Lines'
...
export default function HolidayCalendar(p: { ctx: TypeContext<HolidayCalendarEntity> }): React.JSX.Element | string{
  ...
  const countries = useAPI(() => SchedulerClient.API.getCountries(), []);

  const subDivisions = useAPI(() => ctx.value.countryCode == null ? Promise.resolve([]) :
    SchedulerClient.API.getSubDivisions(ctx.value.countryCode), [ctx.value.countryCode], { avoidReset: true });
