...
import { EngineMessage, Entity, getToString, isEntity, isLite, Lite, toLite } from '@framework/Signum.Entities'
import { EntityLink } from '@framework/Search'
import Alert from './Templates/Alert'
import { getQueryKey, getTypeInfo, ISymbol, PropertyRoute, symbolNiceName } from '@framework/Reflection'
...
export namespace AlertsClient {
  ...
  export function start(options: { routes: RouteObject[], showAlerts: (typeName: string, when: "CreateAlert" | "QuickLink") => boolean }): void {
    ...
    QuickLinkClient.registerGlobalQuickLink(entityType => Promise.resolve([new QuickLinkExplore(entityType, ctx => (AlertEntity.findOptions(token => ({ filterOptions: [token(e => e.target).filter("EqualTo", ctx.lite)] }))),
      {
    ...
    var cellFormatter = new Finder.CellFormatter((cell, ctx) => {
      ...
      var alert: Partial<AlertEntity> = {
        ...
        targetToString: ctx.searchControl?.getRowValue(ctx.row, AlertEntity.token(a => a.targetToString)),
    ...
    Finder.addSettings({
      ...
      hiddenColumns: [
        ...
        { token: AlertEntity.token(a => a.targetToString) },
  ...
  export function format(text: string, alert: Partial<AlertEntity>, onNavigated?: () => void): React.ReactElement {
    ...
    for (const match of Array.from(text.matchAll(LinkPlaceholder))) {
      ...
      if (groups.url) {
        ...
        var toString = getToString(lite);
        var isDeleted = toString == `[${EngineMessage._01NotFound.niceToString(getTypeInfo(lite.EntityType).niceName, lite.id)}]`;
        if (groups.text)
          nodes.push(isDeleted ? <strong>{alert.targetToString}</strong> : <EntityLink lite={lite} onNavigated={onNavigated} > {replacePlaceHolders(groups.text, alert)}</EntityLink>);
        else
          nodes.push(isDeleted ? <strong>{alert.targetToString}</strong> : <EntityLink lite={lite} onNavigated={onNavigated} />);
      }
