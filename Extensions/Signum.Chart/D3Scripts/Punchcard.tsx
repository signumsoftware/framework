...
export default function renderPunchcard({ data, width, height, parameters, loading, onDrillDown, initialLoad, chartRequest, dashboardFilter }: ChartScriptProps): React.ReactElement<any> {
  ...
  return (
    <svg direction="ltr" width={width} height={height} role="group">
      <title>{ChartMessage._0Of1_2.niceToString(symbolNiceName(D3ChartScript.Punchcard), getQueryNiceName(chartRequest.queryKey), [verticalColumn.title, horizontalColumn.title].join(", "))}</title>
