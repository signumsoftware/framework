...
export default class MarkermapChart extends React.Component<{ data: ChartClient.ChartTable, onDrillDown: (e: ChartRow) => void }> {
  ...
  drawChart(data: ChartClient.ChartTable) {
    ...
    if (colorScaleColumn != null) {
      var scaleFunc = scaleFor(colorScaleColumn, data.rows.map(r => colorScaleColumn!.getValue(r)), 0, 100, data.parameters["ColorScale"]);
    ...
    var centerMap = new google.maps.LatLng(
      data.rows.length > 0 ? latitudeColumn.getValue(data.rows[0]) : 0,
      data.rows.length > 0 ? longitudeColumn.getValue(data.rows[0]) : 0)
        ...
        if (infoColumn) {
          ...
          marker.addListener("click", () => {
            ...
            var html = `<div>
                                ...
                                <a ´${(data.parameters["InfoLinkPosition"] == "Inline" ? " style='margin-left: 10px;'" : "")}>
