...
export default function LoginDropdown(p: {
  ...
  if (!user)
    return (
      <LinkContainer to="~/auth/login" className="sf-login">        
        <Nav.Link><i className="sf-login-custom-icon"></i>{LoginAuthMessage.Login.niceToString()}</Nav.Link>
      </LinkContainer>
    );
  ...
  return (
    <NavDropdown className="sf-login-dropdown" id="sfLoginDropdown" title={<span className="d-inline-flex align-items-center"><i className="sf-login-custom-icon sf-login-custom-icon-logged"></i><SmallProfilePhoto user={toLite(user)} /> &nbsp;{p.renderName ? p.renderName(user) : user.userName!}</span>} align="end">
