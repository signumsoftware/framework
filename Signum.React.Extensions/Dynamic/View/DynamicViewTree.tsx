...
export class DynamicViewTree extends React.Component<DynamicViewTreeProps, DnamicViewTreeState>{
    ...
    renderContextualMenu() {
        ...
        return (
                ...
                {no.isContainer && cn.children.length > 0 && <DropdownItem onClick={this.handleClearChildren} className="danger"><i className="fa fa-trash" aria-hidden="true"></i>&nbsp; {DynamicViewMessage.ClearChildren.niceToString()}</DropdownItem>}

                {!isRoot && <DropdownItem divider={true} />}
                {!isRoot && <DropdownItem onClick={this.handleRemove} className="danger"><i className="fa fa-times" aria-hidden="true"></i>&nbsp; {DynamicViewMessage.Remove.niceToString()}</DropdownItem>}
                </ContextMenu>
        );
    }
