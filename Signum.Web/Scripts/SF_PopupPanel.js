...
function NewPopup(urlController, divASustituir, prefix, onOk, onCancel, isEmbeded, detailDiv, partialView) {
	...
	if ($('#' + prefix + sfImplementations).length == 0) $('#' + prefix + sfRuntimeType).val($('#' + prefix + sfStaticType).val());
...
function NewDetail(urlController, divASustituir, prefix, detailDiv, isEmbeded, partialView) {
	...
	if (runtimeType.val() == "") runtimeType.val($('#' + prefix + sfStaticType).val());
...
function TypedOpenPopup(urlController, onOk, onCancel, divASustituir, prefix, typeName, hasImplementations, detailDiv, partialView) {
	...
	var viewQueryParam = "";
	if (partialView != undefined && partialView != null && partialView != "") viewQueryParam = "&sfUrl=" + partialView;
	$.ajax({
		type: "POST",
		url: urlController,
		...
		success: function (msg) {
			window[prefix + sfEntityTemp] = containedEntity;
			if (detailDiv != null && detailDiv != "") $('#' + detailDiv).html(msg);
			else $('#' + prefix + sfEntity).html(msg);
			ShowPopup(prefix, prefix + sfEntity, "modalBackground", "panelPopup", detailDiv);
...
function ReloadEntity(urlController, prefix) {
	var formChildren;
	if ($('#' + prefix + "panelPopup").length != 0) formChildren = $('#' + prefix + "panelPopup *, #" + prefix + sfId + ", #" + prefix + sfRuntimeType + ", #" + prefix + sfStaticType + ", #" + prefix + sfIsNew);
	else formChildren = $("form");
	$.ajax({
		type: "POST",
		url: urlController,
...
function OnListPopupOK(urlController, prefix, btnOkId) {
	...
	if (runtimeType.val() == "") runtimeType.val($('#' + itemPrefix + sfStaticType).val());
}
...
function NewListOption(prefix, selectedType, isEmbeded, detailDiv) {
	...
	staticType.after("<input type=\"hidden\" id=\"" + prefix + newIndex + sfRuntimeType + "\" name=\"" + prefix + newIndex + sfRuntimeType + "\" value=\"" + selectedType + "\" />\n" + "<script type=\"text/javascript\">var " + prefix + newIndex + sfEntityTemp + " = \"\";</script>\n");
	var sfEntityDiv = "<div id=\"" + prefix + newIndex + sfEntity + "\" name=\"" + prefix + newIndex + sfEntity + "\" style=\"display:none\"></div>\n";
	if (detailDiv == null || detailDiv == "") staticType.after(sfEntityDiv);
	else $("#" + detailDiv).html($("#" + detailDiv).html() + sfEntityDiv);
	if (isEmbeded == "False") staticType.after("<input type=\"hidden\" id=\"" + prefix + newIndex + sfId + "\" name=\"" + prefix + newIndex + sfId + "\" value=\"\" />\n");
...
function OnListImplementationsOk(urlController, divASustituir, prefix, onOk, onCancel, isEmbeded, detailDiv) {
	var selectedType = $('#' + prefix + sfImplementationsDDL + " > option:selected");
	if (selectedType.length == 0 || selectedType.val() == "") return;
...
function OnPopupCancel(prefix) {
	...
	var id = $('#' + prefix + sfId);
	if (id.length > 0 && id.val() != null && id.val() > 0) toggleButtonsDisplay(prefix, true);
...
function OnListPopupCancel(btnCancelId) {
	...
	var id = $('#' + prefix + sfId);
	if (id.length > 0 && id.val() != null && id.val() > 0) {} else { if (oldValue != undefined && oldValue != null && oldValue != "") {
			{}
		}
		else {
			$('#' + prefix + sfId).remove();
...
function NewRepeaterElement(urlController, prefix, runtimeType, isEmbedded, removeLinkText, maxElements) {
	if (maxElements != null && maxElements != "") {
		...
		if (elements >= parseInt(maxElements)) return;
	}
